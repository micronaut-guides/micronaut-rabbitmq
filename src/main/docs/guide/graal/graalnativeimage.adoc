...
## Generate GraalVM native image for analytics

To generate the native image you need to generate the FAT JAR first.

[source, bash]
----
analytics $ ./gradlew assemble
----
...
[source, bash]
----
userecho $ native-image --no-server -cp build/libs/analytics-0.1-all.jar
...
## Generate GraalVM native image for books

[source, bash]
----
books $ ./gradlew assemble
----

[source, bash]
----
gateway $ native-image --no-server -cp build/libs/books-0.1-all.jar
